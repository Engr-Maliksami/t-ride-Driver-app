...
class AvailableRideRequestsScreen extends StatefulWidget {
  const AvailableRideRequestsScreen({super.key});

  @override
  State<AvailableRideRequestsScreen> createState() =>
      _AvailableRideRequestsScreenState();
}

class _AvailableRideRequestsScreenState
    extends State<AvailableRideRequestsScreen> {
  final RidesRepository _ridesRepository = RidesRepository();
  List<ActiveRideCourierOrder> _rides = [];
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await _ridesRepository.getActiveRideOrders();
      if (!mounted) return;
      setState(() {
        _rides = list;
        _loading = false;
      ...
      setState(() {
        _rides = [];
        _loading = false;
        _errorMessage = 'could_not_load_ride_requests'.tr;
      });
    }
  }
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return _RideRequestCard(
          ...
          onOpen: () async {
            HapticFeedback.lightImpact();
            await Get.to(() => FindingRideRequestsScreen(ride: ride));
            if (mounted) _load();
          },
        );
      },
...
String _rideStatusPillLabel(String? apiStatus) {
...
({Color background, Color foreground}) _rideStatusPillColors(
  String? apiStatus,
) {
...
class _RideRequestCard extends StatelessWidget {
  const _RideRequestCard({required this.ride, required this.onOpen});
  ...
  Widget build(BuildContext context) {
    ...
    final pillStyle = _rideStatusPillColors(ride.status);
    final pillLabel = _rideStatusPillLabel(ride.status);
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ride.dropoffAddress ?? 'dropoff'.tr,
                style: TextStyle(
                  color: AppConst.blackWithOpacity(0.85),
                  fontSize: 13.sp,
                  fontWeight: FontWeight.w600,
